...
\begin{enumerate}
    \item \textbf{RQ1:} Can integrating operational context signals (scheduled events, deployments, time-of-day patterns) into anomaly classification reduce false positives by $>$40\% while maintaining $>$90\% fault recall?
    \item \textbf{RQ2:} How does the proposed Context Consistency Loss compare to standard cross-entropy for training context-aware classifiers?
    \item \textbf{RQ3:} Which context features contribute most to false positive reduction, and how does performance degrade as context availability decreases?
\end{enumerate}
...
\begin{enumerate}
    \item \textbf{Context-Aware Anomaly Attribution:} The first framework to explicitly distinguish workload-induced anomalies from actual faults using operational context signals within a unified classification pipeline.
    
    \item \textbf{Context Consistency Loss:} A composite loss function with gated unknown-context penalty and TADAM-style delta regularization, preventing FiLM noise amplification on small datasets.
    
    \item \textbf{Context Integration Module:} FiLM conditioning with skeptical confidence gating (bias $= -1.0$, initial gate output $\approx 0.27$) and feature-group dropout for noise robustness.
    
    \item \textbf{Piecewise Linear Embeddings (PLE):} Quantile-based bin encodings that give MLPs axis-aligned threshold capability, closing the neural-tree gap to 1.0\% on synthetic data.
    
    \item \textbf{CAAA+CatBoost Hybrid:} FiLM-conditioned neural embeddings concatenated with raw features, classified by CatBoost---the first work combining FiLM conditioning with tree-based downstream classifiers.
    
    \item \textbf{Evaluation Methodology:} Severity-tiered synthetic data with disguised faults, pre-injection split for distribution-matched RCAEval evaluation, counterfactual baselines via RNG forking, and label leakage prevention via randomized context assignment.
\end{enumerate}

\section{Report Organization}

The remainder of this report is organized as follows: Chapter~2 reviews related work across anomaly detection, root cause analysis, FiLM conditioning, and tabular deep learning. Chapter~3 details the proposed methodology including architecture design, loss functions, and hybrid approaches. Chapter~4 presents the evaluation methodology, a first-class contribution addressing three evaluation pitfalls. Chapter~5 presents comprehensive experimental results. Chapter~6 provides discussion, error analysis, and limitations. Chapter~7 concludes with lessons learned and future directions.
...
\begin{table}[h]
    ...
    \begin{tabular}{lcccccc}
        \toprule
        \textbf{Method} & \textbf{Met} & \textbf{MM} & \textbf{Ctx} & \textbf{FiLM} & \textbf{Attr} & \textbf{FPR} \\
        \midrule
        ...
        \sys{} (Ours) & $\checkmark$ & -- & $\checkmark$ & $\checkmark$ & $\checkmark$ & $\checkmark$ \\
        \bottomrule
    \end{tabular}

    \smallskip
    \footnotesize Met=Metrics, MM=Multimodal, Ctx=Context, Attr=Attribution, FPR=FP Reduction
\end{table}
