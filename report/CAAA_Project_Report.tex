...
at large $n$ and a lowered saturation plateau, the Full \sys{} model reaches
$95.11\% \pm 0.18$ F1 versus $94.90\% \pm 0.21$ for CatBoost-with-context
(Welch's $t = 2.249$, $p = 0.0375$). Second, and more robustly,
...
\begin{enumerate}
    ...
    \item \textbf{Empirical evidence of a neural-over-tree crossover on anomaly attribution:} At $n = 40{,}000$ on the harder synthetic task, Full \sys{} reaches $95.11\% \pm 0.18$ F1 versus $94.90\% \pm 0.21$ for CatBoost~with~context (Welch's $t = 2.249$, $p = 0.0375$). We report the effect size honestly: this is a statistically detectable but practically modest advantage that confirms a crossover regime rather than large architectural dominance.
...
Perez~\textit{et al.}~\cite{perez2018film} introduced Feature-wise Linear Modulation (FiLM) for visual reasoning, demonstrating that conditioning neural networks on auxiliary information via learned affine transformations is highly effective. Oreshkin~\textit{et al.}~\cite{oreshkin2018tadam} proposed TADAM, initializing FiLM gamma as $1 + \delta$ with $\delta$ starting at zero, constraining the multiplicative path to near-identity---precisely to prevent uncontrolled multiplicative signal amplification early in training when the conditioning stream is still noisy. Our skeptical-initialization and unit-norm-gated design (Chapter~3) adopts this rationale directly.
...
We introduce a \emph{Context Consistency Loss} (CCL) designed for our specific setting: penalising predictions that contradict the context signal while leaving cases with unreliable or missing context unpenalised. The formulation is novel in combining (i) confidence-weighted consistency between prediction and context prior, (ii) context-conditioned entropy regularisation, and (iii) a gated unknown-context penalty. The underlying principle---regularising a classifier so its outputs remain consistent with an auxiliary signal---follows the consistency-training tradition established by FixMatch~\cite{sohn2020fixmat} and unsupervised data augmentation~\cite{xie2020uda} in semi-supervised learning; we adapt it from augmentation-invariance to context-invariance.
...
We also explored a two-stage hybrid: (1) the trained \sys{} model produces 64-dim FiLM-conditioned embeddings, (2) these are concatenated with raw 44-dim features, and (3) CatBoost classifies the 108-dim input. On real data (Table~\ref{tab:real-pooled}) the hybrid reaches F1~$=0.9328$---worse than plain CatBoost ($0.9542$), meaning the FiLM embedding adds noise more than signal at this scale. We include the hybrid in Chapter~5 for completeness and drop it from the scaling study and universal-context analysis in Chapters~6--7, where it is dominated by both Full \sys{} and CatBoost with raw context features.
...
\begin{itemize}
    ...
    \item \textbf{Controlled difficulty.} To distinguish capacity from task saturation, we need the ability to lower the empirical saturation plateau. No public benchmark offers this lever.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Default difficulty}---severity distribution $\{0.35, 0.35, 0.30\}$ over \{\textsc{low}, \textsc{medium}, \textsc{high}\}; severity factors $\{0.05, 0.30, 1.00\}$ scaling metric deviations; context-noise rate $0.30$. This regime has a empirical saturation plateau near $97\%$ F1.
...
\paragraph{Residual epistemic risk.} We cannot prove the absence of a fourth, undetected leakage path---this is an inherent limitation of any synthetic-data evaluation. We can, however, bound how much of the observed performance could be explained by residual leakage. Chapter~5's \textsc{context\_only} baseline---a model trained exclusively on the five context features, with all fault features removed---reaches $78.8\%$ F1 on real data (Table~\ref{tab:real-pooled}) and comparable levels on synthetic data. This figure is an empirical ceiling on what any context-label shortcut can deliver: if a hidden leakage channel existed, it would express itself through the context features, and \textsc{context\_only} would approach $100\%$. The gap between $78.8\%$ and Full \sys{}'s $95.1\%$ at $n = 40{,}000$ hard samples is at least $16$ percentage points of genuine fault signal that context alone cannot explain. A reviewer treating this as the conservative residual-leakage bound is, in our view, warranted.
...
\section{Aggregate Real-Data Results}

Table~\ref{tab:real-pooled} summarises performance as a macro-average across the nine valid system-by-dataset cells (each cell's seed-mean F1 is computed first, then the nine cell means are averaged with equal weight). This gives every system equal weight regardless of its sample count, avoiding domination by the larger cells. All ``pooled'' figures in the subsequent text refer to this cell-macro-average, \emph{not} a sample-weighted pooling.

\begin{table}[!htbp]
    \centering
    \caption{Aggregate RCAEval Results (Macro-Averaged Over 9 System/Dataset Cells, 10 seeds each)}
...
\begin{table}[!htbp]
    ...
    \begin{tabular}{llccccc}
        \toprule
        \textbf{DS} & \textbf{System} & \textbf{Full \sys{}} & \textbf{No Ctx} & \textbf{CatBoost} & \textbf{Cat+Ctx} & \textbf{$\Delta$} \\
        \midrule
...
The single largest cell-level deficit is RE3/OnlineBoutique ($-12.6$ pp, Full \sys{} $81.5$ vs.\ CatBoost$+$ctx $94.1$). RE3 contains only $90$ code-level fault cases in total, so a single system's cell holds roughly $30$ cases, of which $\sim$$10$ land in the held-out test split per seed. At that scale, a handful of mispredictions on the neural model's side moves the F1 by many percentage points, and the gap is dominated by per-seed test-set composition rather than an architectural difference that would persist at larger $n$. Chapter~6 reruns the same head-to-head under controlled scaling and shows this deficit does not survive once sample size reaches the low thousands.
...
\begin{itemize}
    \item \textbf{H1 (Saturation hypothesis):} on the default-difficulty synthetic task, all model families will converge toward a shared empirical saturation plateau as $n$ grows. Architecture differences become undetectable because the task becomes trivial.
...
\begin{figure}[!htbp]
    ...
    \end{axis}
    \end{tikzpicture}
    \caption{Default-difficulty scaling: F1 vs.\ total sample size. Both \sys{} and CatBoost-with-context saturate near $97\%$ F1; the gap at 40{,}000 is $+0.16$ pp (within seed-level noise, $p=0.09$). Each point is a mean over 3 seeds except $n=40{,}000$ which uses 10 seeds; error bars omitted for readability, per-size standard deviations are tabulated in Table~\ref{tab:scaling-default}.}
...
Lowering the saturation plateau changes the picture. Figure~\ref{fig:scaling-hard} shows the hard-difficulty scaling curves; Table~\ref{tab:scaling-hard} reports numerical values for the same head-to-head comparison.
...
\begin{figure}[!htbp]
    ...
    \end{axis}
    \end{tikzpicture}
    \caption{Hard-difficulty scaling: F1 vs.\ total sample size. The empirical saturation plateau drops to $\sim$$95\%$. Full \sys{} overtakes CatBoost-with-context at $n \approx 20{,}000$ and opens a statistically significant $0.21$ pp gap at $n = 40{,}000$ (Welch's $t{=}2.249$, $p{=}0.0375$). Each point is a mean over 3 seeds except $n=40{,}000$ which uses 10 seeds; error bars omitted for readability, per-size standard deviations are tabulated in Table~\ref{tab:scaling-hard}.}
...
This $p$-value should be read as evidence for a single pre-specified contrast---Full \sys{} versus the best tree baseline with context---designated as the primary comparison before the hard-difficulty run based on Chapter~5's winner. It is not a multiple-comparison-corrected statement over all pairwise contrasts in Tables~\ref{tab:scaling-full-hard} or~\ref{tab:context-contribution}; if a reader applied a Bonferroni correction over the five ``$+$ context vs.\ $-$ context'' family contrasts of Chapter~7, the Chapter~7 effects would remain strongly significant ($p \ll 10^{-3}$ for every family) while this H2 contrast would sit at adjusted $p \approx 0.19$. We therefore treat the crossover as \emph{suggestive} evidence of the architectural effect (the real load-bearing claim lives in Chapter~7) and report it at this effect size rather than inflating it.
...
\begin{center}
    ...
    \begin{tabular}{lc}
        ...
        \bottomrule
    \end{tabular}
    \captionof{table}{All Variants at 40{,}000 Hard Samples (10 seeds, mean $\pm$ std)}
...
\begin{figure}[!htbp]
    ...
    \node[pin={[pin edge={red!70!black,thick,-latex}, font=\scriptsize, text=red!70!black, pin distance=10mm]above right:Neural--tree gap (0.21 pp)}] at (axis cs:0.21,RandomForest) {};
    \end{axis}
    \end{tikzpicture}
...
\bibitem{sohn2020fixmat}
K.~Sohn \emph{et al.}, ``FixMatch: Simplifying Semi-Supervised Learning with Consistency and Confidence,'' in \emph{Proc. NeurIPS}, 2020.

\bibitem{xie2020uda}
Q.~Xie \emph{et al.}, ``Unsupervised Data Augmentation for Consistency Training,'' in \emph{Proc. NeurIPS}, 2020.
